...
import { ArrowLeft, ArrowRight, Shield, Sparkles, AlertCircle, CheckCircle2, Info, Compass, Layers } from "lucide-react";
...
// Since this is a dynamic route in App Router, we handle params
export default function DynamicHealthPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = use(params);
  
  // Logic to parse slug: e.g. "bloating-vata"
  const combo = COMBINATIONS[slug];
  
  if (!combo) {
    // If no specific combo, check if it's just a symptom
    // In our programmatic strategy, we might redirect to a default or show notFound
    notFound();
  }

  // Extract component parts for content enrichment
