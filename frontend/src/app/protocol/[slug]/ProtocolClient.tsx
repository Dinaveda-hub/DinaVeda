...
import { ArrowLeft, ArrowRight, ShieldCheck, Clock, AlertTriangle, Bookmark, Heart, Sparkles, Activity } from "lucide-react";
...
function ProtocolJSONLD({ name, mechanism }: { name: string; mechanism: string }) {
  const jsonLd = {
    ...
    "@graph": [
      ...
      },
      {
        ...
        "medicalSpecialty": "TraditionalMedicine",
        "recognizingAuthority": "Ayurveda",
...
function ProtocolNav() {
  return (
    <nav className="p-6 flex justify-between items-center max-w-7xl mx-auto sticky top-0 bg-white/80 backdrop-blur-md z-[70] border-b border-slate-50">
      ...
      <Link href="/" className="font-black text-forest text-xl tracking-tighter" aria-label="Dinaveda Home">
        Dinaveda
      </Link>
      <Link 
        href="/login" 
        ...
        aria-label="Go to biological assessment"
      >
        Assessment
      </Link>
    </nav>
...
function ProtocolHeader({ name, duration, raw }: { name: string; duration: string; raw: any }) {
  return (
    <motion.header 
...
function ProtocolMechanism({ name, mechanism, raw }: { name: string; mechanism: string; raw: any }) {
  return (
    <div className="bg-white p-8 md:p-10 rounded-3xl md:rounded-[3.5rem] border border-slate-100 shadow-sm relative overflow-hidden">
...
function MetabolicImpact({ name }: { name: string }) {
  return (
    <div className="bg-white p-8 md:p-10 rounded-3xl md:rounded-[3.5rem] border border-slate-100 shadow-sm relative overflow-hidden">
...
function ProtocolSteps({ bundle }: { bundle: any }) {
  if (!bundle?.steps) return null;
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4">
        {(bundle.steps ?? []).map((step: any, idx: number) => (
          <div key={idx} className="bg-white p-8 rounded-3xl md:rounded-[2.5rem] border border-slate-100 flex gap-6 items-start hover:shadow-lg transition-all border-l-4 border-l-forest">
...
function SafetySection({ contraindications }: { contraindications: string[] }) {
  if (!contraindications || contraindications.length === 0) return null;
  return (
    <div className="bg-orange-50/50 p-8 md:p-10 rounded-[2.5rem] md:rounded-[3rem] border border-orange-100">
...
function CTASection() {
  return (
    <div className="bg-forest p-10 md:p-16 rounded-3xl md:rounded-[4rem] text-white text-center relative overflow-hidden shadow-3xl shadow-forest/20">
      <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-400 blur-3xl opacity-10 pointer-events-none -mr-20" />
      ...
      <Link 
        ...
        aria-label="Start tracking your metabolic pulse"
      >
...
export default function ProtocolClient({ name, mechanism, duration, indications, contraindications, raw, bundle }: ProtocolClientProps) {
  return (
    <div className="bg-[#FAFBFB] text-slate-800 min-h-screen relative font-sans overflow-x-hidden selection:bg-forest/20 selection:text-forest">
      <ProtocolJSONLD name={name} mechanism={mechanism} />

      <ProtocolNav />

      <main className="max-w-4xl mx-auto px-6 py-16 md:py-20 lg:py-24">
        <ProtocolHeader name={name} duration={duration} raw={raw} />

        <section className="space-y-12 md:space-y-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             <div className="col-span-1 md:col-span-2 space-y-8">
                <ProtocolMechanism name={name} mechanism={mechanism} raw={raw} />
          ...
          {bundle ? (
            <ProtocolSteps bundle={bundle} />
          ) : (
            <div className="bg-white p-10 md:p-12 rounded-3xl md:rounded-[4rem] border border-slate-100 text-center space-y-8 shadow-premium overflow-hidden relative">
          ...
          <SafetySection contraindications={contraindications} />
