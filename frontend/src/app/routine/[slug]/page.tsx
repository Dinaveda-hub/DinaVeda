...
import { ArrowLeft, ArrowRight, Sun, Moon, Clock, Utensils, Droplet, Activity, Sparkles, BookOpen, Calendar, Repeat, Coffee, Sunrise, Sunset, Wind, Zap } from "lucide-react";
...
export default function RoutinePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Try to find in manual routines first
  let routine = ROUTINES[slug];
  let isClinical = false;

  // Virtual Routine Generation logic if slug ends with -clinical
  if (!routine && slug.endsWith("-clinical")) {
    const timeOfDay = slug.split("-")[0]; // "morning", "midday", "evening"
    ...
    if (filteredProtocols.length > 0) {
      ...
      routine = {
        ...
        category: "daily",
        blocks: filteredProtocols.map((p, idx) => {
          // Approximate timing based on index for the visual flow
          ...
          return {
            ...
            icon: p.category.includes('digestive') ? Utensils : p.category.includes('vata') ? Wind : p.category.includes('kapha') ? Zap : Activity
          };
        })
      };
    }
